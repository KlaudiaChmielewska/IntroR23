...
Before, however, here's a skeleton for a function. You'll see the implementation of it allowing for a faster code activation.
\

``` {r function_one, eval = FALSE}
...
So consider that if you put any of the following statements, they can be automatized in a function, which small programs will exemplify you more.
\
\
...
# *<span style="text-decoration:underline">Statements</span>*
\
\
...
The function print() will print you the information in parenthesis to the console.
\
...
What if you wanted to check the if statement more than once, we'd have to change the value of a and b every time before we activate the code again. 
\

You can add a function, which means now you'd only have to write function(a,b) to get the result.


``` {r if_one_function, eval=FALSE}
...
We can avoid having to write anything else than the  value of a and b - by adding small elements to our code in order to allow us to see the prompt automatically coming back, asking to ![](value_a.png) and further - ![](value_b.png) and print us which value is greater than the other. You can see however that instead if print() function now, I used cat() in order to print both the string and the values attributed both to a and b.

\

You can also see a 'while' (TRUE) which in this particular situation means the console will ask you to provide 'a' and 'b' all the time, in an infinite loop, not stopping until you click Esc.
\
\


``` {r if_one_program, eval=FALSE}
...
A good representation of if/else if statement is pH. Defining whether the pH is acidic, neutral or basic has its simple rules:

* if it's between 1-6 it's acidic 
* if it's between 8-14 it's basic
* if it's 7 it's neutral
\

This is exemplified by the code below, designed to already ask you for input all the time (or until you click Esc!)
\

Examine the lines and try to understand each line. Focus on how the conditions are written.

``` {r if_two_program, eval=FALSE}
while (TRUE) {
...
<button class="button5"> \%\in\% </button>
<button class="button5"> \%\*\% </button> 
